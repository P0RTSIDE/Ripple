...
class Fish {
    ...
    draw(ctx) {
        ...
        // Lifted fish: shadow stays on the water plane, body rises above it.
        if (liftZ > 1 && !this.golden) {
            ctx.save();
            ctx.fillStyle = `rgba(0,0,0,${0.18 + 0.12 * Math.min(1, liftZ / 50)})`;
            ctx.beginPath();
            ctx.ellipse(this.x, this.y, L * 0.42, L * 0.22, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
        }
